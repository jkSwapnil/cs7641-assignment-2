""" 
Main function for neural network training using Randomized Hill Climbing
"""
...
RESULT_DIR = "./results/nn_rhc"
PLOT_DIR = "./plots/nn_rhc"
np.random.seed(RANDOM_STATE)


def nn_rhc():
    """ Function to implement neural network training using Randomized Hill Climbing """
    print("Training neural network using Randomized Hill Climbing")
    print("- - - - - - - - - - - - - - - - - - - - - - - - - - - ")
    ...
    for i in tqdm(range(0, 81)):
        ...
        rhc_best_state, rhc_best_fitness, _ = mlrose.random_hill_climb(
            problem=problem, max_attempts=1000, max_iters=max_iters, 
            init_state=cur_state, curve=True, random_state=RANDOM_STATE
            )
        training_fitness.append(rhc_best_fitness)
        ...
        y_pred = feed_forward(  # Generate label predictions on test data
            state=rhc_best_state, input_dim=input_dim, hidden_dim=hidden_dim, 
